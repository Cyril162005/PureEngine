...
**Notes:**
- Approach ruling: option (a) — per-type IN-TREE generated textures, chosen over tint-only recoloring. Per-entity texture binding was exactly the seam Step 12's own comment deferred ('Per-entity textures remain Step 12's seam, deliberately deferred'); resolving it is an index→texture select inside the existing draw loop with zero shader/sampler/pipeline change. Tint-only would leave every entity sharing one pattern — weaker threat-vs-decoration readability, which is the phase's usability point.
- New assets: three 16x16 RGB PNGs generated by make_textures.ps1 (same hand-written PNG machinery as make_checker.ps1 — CRC32, big-endian chunks, zlib-wrapped deflate, color type 2), 87 bytes each: tex_player.png warm green, tex_scenery.png steel blue, tex_hostile.png crimson. Threat readable from decoration at a glance.
- TINT RULE: every palette color keeps red-channel content — the Step 8 collision tint multiplies the texel by (1,0,0), so a zero-red texture would render BLACK on collision. Hostiles are never tinted (colliding flags are only set inside the scenery loop), so crimson is free for them.
- Code: a loadRgbTexture lambda extracts the checker's load/upload pattern (third copy by Phase 3) and is called three times, each with the 3-candidate CWD probe; the three textures join ALL cleanup chains (own failure path, font-failure, texLocation-failure, final teardown); draw loop binds per entity (index 0 → player, 1..2 → scenery, 3+ → hostiles — the collision loops' existing index convention). The checker stays loaded as legacy, sampled by no entity anymore. Collision tint, beep edge-detection, difficulty ramp, and Phase 3/4 UI untouched.
...
**Notes:**
- Ruling: documented folder structure + README, no installer — honest for a solo learning project. README.md created at repo root: project intro (12 engine steps + 6 game phases, tracker link), requirements, the EXACT two CMake commands used throughout the project, asset-generator table, run instructions (3-candidate probe), controls, save-file note, and the standalone layout.
- DLL audit (honest): GLFW/miniaudio/stb/GLAD were already statically linked, but MSVC defaulted to /MD — the exe depended on VCRUNTIME140.dll/MSVCP140.dll, absent on a clean machine. First fix attempt (CMAKE_MSVC_RUNTIME_LIBRARY + CMP0091) FAILED silently: the fetched dependencies declare their own cmake_minimum_required, resetting the policy scope (generated project files proved /MD survived). Replaced with the scope-proof flag rewrite (/MD→/MT across all CMAKE_C/CXX_FLAGS* variables) — 25 lines in CMakeLists.txt, the phase's only build change.
- Verification: dumpbin /DEPENDENTS now lists ONLY KERNEL32, USER32, GDI32, SHELL32 — zero runtime DLLs to ship. Live test: the exe launched from a bare folder containing only itself + assets/ (no savedata/, simulating a clean machine) and ran past every startup check. Exe grew 838,144 → 1,170,432 bytes (static runtime). Package = exe + assets/ only; savedata/ intentionally not shipped (fresh installs start at 0.0).
- Build note: full build-folder delete + fresh configure (757.9 s) + Release build clean FIRST TRY, zero engine-code warnings; incremental reconfigure after the /MT fix (19.2 s); main.cpp 1,565 → 1,675 lines. Phases 5+6 shipped in one combined pass as the tracker's final work — GAME BUILD COMPLETE, 6/6 phases.
