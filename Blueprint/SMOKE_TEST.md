...
| 7.9 | Opt-in 3D debug mesh (Step 194) | **HOW TO INVOKE (Step 202): launch Arcade → press backtick (`` ` ``) to open the console → type `debug3d on` → ENTER.** The unit cube appears over the 2D frame (perspective + depth composition); `debug3d off` → 2D-only rendering restored exactly (the Step 193/196 mode round-trip). The CPU-side data (36×5 vertex layout, unit bounds, translation builder) is CI-proven — the on-screen pixels are HUMAN-ONLY |
