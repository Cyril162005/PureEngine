...
| 7.9 | Opt-in 3D debug mesh (Step 194) | **HOW TO INVOKE (Step 202): launch Arcade → press backtick (`` ` ``) to open the console → type `debug3d on` → ENTER.** The unit cube appears over the 2D frame (perspective + depth composition); **bind a LIVE entity (Step 219): `meshid 1 1` → a cube appears at the HOSTILE's position (index 1, the one chasing you) and FOLLOWS it as it moves — the physics→3D proof: play normally and watch the cube chase**; move the camera (Step 213): `debug3d cam <ex> <ey> <ez> <tx> <ty> <tz>` (e.g. `debug3d cam 3 2 5 0 0 0`) → the cube renders from a new angle; `debug3d off` → 2D-only rendering restored exactly (the Step 193/196 mode round-trip). The CPU-side data (36×5 vertex layout, unit bounds, translation builder, the per-frame refresh) is CI-proven — the on-screen pixels are HUMAN-ONLY |
