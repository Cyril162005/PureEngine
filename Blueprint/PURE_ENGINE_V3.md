...
- **Packager drift F-08 (2026-09-12):** `scripts/package.ps1` is single source of truth for shipped arcade zip (14 files + `assets/shaders/`). `CMakeLists.txt` POST_BUILD arcade bundle mirrors it for dev runs. Verified: clean-dir zip (`package/PureEngine-0.1.0-win64.zip`) contains `arcade_arena.txt`, `animation_default.txt`, `paddle_spritesheet.png` + shaders, runs from extracted dir without repo fallback (3s alive). No other systems touched.
